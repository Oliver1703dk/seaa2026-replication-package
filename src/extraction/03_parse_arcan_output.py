#!/usr/bin/env python3
"""Parse Arcan CSV outputs into a smell summary CSV.

Arcan 2 CLI trial outputs per-snapshot:
  arcanOutput/project/smell-characteristics.csv  - one row per smell
  arcanOutput/project/component-metrics.csv      - one row per component

Reads: data/raw/arcan_output/{dir_name}__{month}/arcanOutput/project/
Writes: data/interim/parsed_smells_summary.csv
"""
import csv
import logging
import random
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(log_dir / "03_parse_arcan_output.log"),
    ],
)
log = logging.getLogger(__name__)

ARCAN_OUT = PROJECT_ROOT / "data/raw/arcan_output"
OUTPUT = PROJECT_ROOT / "data/interim/parsed_smells_summary.csv"
...
def parse_snapshot_id(dirname: str) -> tuple[str, str]:
    """Split 'owner__repo__YYYY-MM' into (dir_name, month)."""
...
def find_arcan_csvs(snapshot_dir: Path) -> Path | None:
    """Find the arcanOutput/project/ directory with CSV files."""
    arcan_subdir = snapshot_dir / "arcanOutput"
    if not arcan_subdir.is_dir():
        return None
    # Arcan puts output in a subdirectory named after the -i path basename
...
def parse_snapshot(csv_dir: Path) -> dict:
    """Extract smell counts and package stats from Arcan CSV output."""
    smells = {"cd": 0, "ud": 0, "hl": 0, "gc": 0}
    num_packages = 0
    total_loc = 0

    # Parse smell counts from smell-characteristics.csv
    ...
    # Parse package stats from component-metrics.csv
...
def main() -> None:
    log.info("Parsing Arcan CSV outputs from %s", ARCAN_OUT)
    ...
    for snap_dir in tqdm(snapshot_dirs, desc="Parsing smells"):
